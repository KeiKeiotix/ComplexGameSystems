...
public static class MeshGenerator
{
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve heightCurve, int levelOfDetail, bool useFlatShading)
    {
        ...
        int meshSimplifyIncrement = (int)Mathf.Pow(2, levelOfDetail);
        ...
        int verticesPerLine = (borderedSize - 1) / meshSimplifyIncrement + 1;
        ...
        for (int y = 0; y < borderedSize; y += meshSimplifyIncrement)
        {
            for (int x = 0; x < borderedSize; x += meshSimplifyIncrement)
            {
                ...
                meshData.AddVertex(vertexPosition, percent + Vector2.one * 1.0f / (float)meshSize, vertexIndex);
